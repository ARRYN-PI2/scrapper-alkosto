...
class AlkostoScraperAdapter(ScraperPort):
    ...
    def _is_product_url(self, url: str, categoria: str) -> bool:
        ...
        # Must contain /p/ to be a product URL
        if "/p/" not in url:
            return False
        
        # Should not be a category URL
        if "/c/" in url:
            return False
    ...
    def _guess_items_from_html(self, html: str, categoria: str) -> List[Dict[str, Any]]:
        ...
        # Try multiple selectors for Alkosto product links
        selectors = [
        ...
        # Remove duplicates and filter for valid product URLs
        seen_links = set()
        unique_links = []
        for link in links_found:
            href = link.get("href", "")
            if href and href not in seen_links and self._is_product_url(href, categoria):
                seen_links.add(href)
                unique_links.append(link)
        
        print(f"Found {len(unique_links)} valid product links after filtering")
    ...
    def _extract_product_details(self, url: str) -> str:
        ...
        try:
            ...
            # Extract key features (características destacadas)
            key_features = soup.select('.tab-details__keyFeatures--item')
            if key_features:
                details.append("Características destacadas:")
                for feature in key_features:
                    feature_text = feature.get_text(strip=True)
                    if feature_text:
                        details.append(f"• {feature_text}")
                details.append("")
            
            # Extract specifications organized by category
            spec_sections = soup.select('.new-container__table__classifications___type__wrap')
            
            for section in spec_sections:
                # Get category title
                title_elem = section.select_one('.new-container__table__classifications___type__title')
                if title_elem:
                    category_title = title_elem.get_text(strip=True)
                    if category_title:
                        details.append(f"{category_title}:")
                
                # Get items in this category
                items = section.select('.new-container__table__classifications___type__item')
                for item in items:
                    # Skip void items
                    item_classes = item.get('class')
                    if item_classes and 'void' in ' '.join(str(cls) for cls in item_classes):
                        continue
                    
                    feature_elem = item.select_one('.new-container__table__classifications___type__item_feature')
                    result_elem = item.select_one('.new-container__table__classifications___type__item_result')
                    
                    if feature_elem and result_elem:
                        feature_text = feature_elem.get_text(strip=True)
                        result_text = result_elem.get_text(strip=True)
                        
                        if feature_text and result_text:
                            # Clean up result text
                            result_text = re.sub(r'&nbsp', '', result_text)
                            result_text = re.sub(r'<br\s*/?>', ' | ', result_text)
                            result_text = re.sub(r'\s+', ' ', result_text)
                            result_text = result_text.strip()
                            
                            details.append(f"• {feature_text}: {result_text}")
                
                details.append("")  # Add spacing between categories
            
            # Extract product description if available
            description_template = soup.select_one('#wc-product-description')
            if description_template:
                desc_text = description_template.get_text(strip=True)
                if desc_text and len(desc_text) > 50:  # Only include if substantial
                    details.append("Descripción del producto:")
                    # Clean and truncate description
                    desc_text = re.sub(r'\s+', ' ', desc_text)
                    if len(desc_text) > 800:
                        desc_text = desc_text[:800] + "..."
                    details.append(desc_text)
    ...
    def scrape(self, categoria: str, page: int) -> Iterable[Producto]:
        ...
        for idx, it in enumerate(items, start=1):
            ...
            # Extract detailed information from product page if link is available
            details_cleaned = ""
            if link and link.startswith("http"):
                details_cleaned = self._extract_product_details(link)
            
            # Fallback to basic details if extraction failed
            if not details_cleaned and details:
                details_cleaned = clean_html_details(details)
